...
import com.bidgely.commonExecutionResources.ExecutionVariables;
import com.bidgely.commonExecutionResources.ExecutionContext;
import com.bidgely.configs.RecreateProdUserDataConfig;
import com.bidgely.commons.BillCycleCodes;
import com.bidgely.responses.entries.GBConsumptionDataResponseEntry;
import com.bidgely.responses.entries.UserManagementUserRegistrationEntry;
import com.bidgely.responses.entries.UtilityBillingDataResponseEntry;
...
public class RecreateProdUserDataTool {
	...
	public void execute(ExecutionContext executionContext,String config, ExecutionVariables executionVariables) throws BidgelyExceptions {
		...
		if(recreateProdUserDataConfig.getDestinationPilotName()!=null && recreateProdUserDataConfig.getDestinationPilotId()!=null){
			...
			utilityName = executionContext.getProperty(ContextKeys.UserDefaults_UtilityName).trim();
			pilotId = executionContext.getProperty(ContextKeys.ExecutionEnvironment_PilotId).trim();
		}
		String sourceUtilityName = executionContext.getProperty(ContextKeys.UserDefaults_UtilityName).trim();
		...
		try {
			environmentProperties.load(RecreateProdUserDataTool.class.getResourceAsStream(
					"/environment/properties/global.properties"));
			String propertyFilePath = environmentProperties.getProperty("email.validation.propertyFilepath") +
					"execution/context/" + utilityName+ "/consolidated.properties";
		...
		meterContractDateFormat.setTimeZone(TimeZone.getTimeZone(executionContext.getProperty(ContextKeys.UserDefaults_Timezone).trim()));
		...
		try {
			for (String userUUID : listOfUsers) {
				...
				if(billCycleCodeOftheProdUser!=null && billCycleCodes.checkIfBillingCycleCodeExists(billCycleCodeOftheProdUser, Integer.parseInt(pilotId))) {
					...
					this.billCycleCode = billCycleCodes.getBillingCycleCodeAt(executionContext, Integer.parseInt(billCycleCodeOftheProdUser), Integer.parseInt(pilotId), utilityName);
				} else{
				...
				InvoiceDataList.MetaUserApiResponse metaUserApiResponse = SharedResources.gson.fromJson(sendGetRequest(metaUserDetailsApi), InvoiceDataList.MetaUserApiResponse.class);
				...
				if (recreateProdUserDataConfig.isConstructUserFile()) {
					...
					if(recreateProdUserDataConfig.isConstructMeterFile()){ //for EM,ameren,amerensmb,edison,ei
						userFileData.append(getUserFileData(userDetailsResponse, executionContext, utilityName, executionVariables, "", "", "", ""));
					}else {
						for(String meterFuel: meterFuelList) {
							String gwsId = getGWSId(meterFuel);
							metaUserHomeGatewaysMeterApi = String.format("/meta/users/%s/homes/1/gws/%s", userUUID, gwsId);
							...
							userFileData.append(getUserFileData(userDetailsResponse, executionContext, utilityName, executionVariables, meterFuel, contractStart, contractEnd, gwsId));
				...
				if (recreateProdUserDataConfig.isConstructInvoiceFile() && invoiceFilePrefix.toLowerCase().contains("opower")){
					...
					for(String meterFuel: meterFuelList) {
						...
						invoiceData.append(getOpowerFileData(utilityBillingDataResponse, userDetailsResponse, executionVariables, executionContext, utilityName, meterType, measurementType, invoiceDateFormat, invoiceValueSeparator, contractStart, contractEnd, gwsId, dateFormatStr));
					}
				...
				if (recreateProdUserDataConfig.isConstructRawFile()) {
					...
					//                    rawFileDateFormat.setTimeZone(TimeZone.getTimeZone(executionContext.getProperty(ContextKeys.UserDefaults_Timezone).trim()));
				...
				if (recreateProdUserDataConfig.isConstructInvoiceFile() && executionContext.getProperty(ContextKeys.Prefix_InvoiceFile).toLowerCase().contains("invoice")) {
					StringBuilder invoiceData= new StringBuilder();
					for(String meterFuel: meterFuelList) {
						String gwsId = getGWSId(meterFuel);
						String meterType = meterFuel.split("-")[0];
	...
	private String getValuesFromToken(String utilityName, String token, String gwsId, ExecutionVariables executionVariables){
		...
		case "ei":
			...
			executionVariables.put("dataStreamId_"+gwsId, dataStreamId);
		...
		case "bhermputres":
			...
			premiseId = token.split("_")[2];
			executionVariables.put("dataStreamId_"+gwsId, SharedResources.generateRandomString("DDDDDDDDD"));
		...
		case "sdgesmb":
			token = token.split(":")[1];
			premiseId = token.split("_")[2];
			executionVariables.put("secondaryId", token.split("_")[0]);
			if(customerId==null) customerId = SharedResources.generateRandomString("DDDDDDDDDDDDD");
			if (executionVariables.get("servicePointIdNumber")==null) {
				String servicePointIdNumber = "000" + SharedResources.generateRandomString("D");
				executionVariables.put("servicePointIdNumber", servicePointIdNumber);
				executionVariables.put("servicePointId", this.premiseId + "-" +servicePointIdNumber);
			}
			metaTokenOFCreatedUser = ":" + executionVariables.get("secondaryId")+"__"+premiseId; //secondaryId__premiseId
			break;
		case "pnm":
			token = token.split(":")[1];
			...
			executionVariables.put("secondaryId", token.split("_")[1]);
			break;
		case "ppl":
			if(customerId==null) customerId =  SharedResources.generateRandomString("DDDDDDDDD");
		...
		case "cva":
			...
			premiseId = token.split("_")[2];
			executionVariables.put("servicePointId", token.split("_")[3]);
			executionVariables.put("secondaryId", SharedResources.generateRandomString("DDDDDDD"));
			metaTokenOFCreatedUser = ":1_"+customerId+"_"+premiseId+"_"+executionVariables.get("servicePointId")+"_NSM";
			break;
		case "pseg":
			token = token.split(":")[1];
			if(customerId==null) customerId = SharedResources.generateRandomString("DDDDDDDDD");
			contractId = token.split("_")[1];
			premiseId = token.split("_")[2];
			executionVariables.put("dataStreamId_"+gwsId, "");
		...
		case "honi":
			...
			executionVariables.put("partnerUserId", token.split("_")[1]);
			executionVariables.put("contractAccountId", SharedResources.generateRandomString("DDDDDDDDDDDDD"));
			executionVariables.put("dataStreamId_"+gwsId,SharedResources.generateRandomString("DDDDDDDDD"));
			metaTokenOFCreatedUser = ":" + token; //contractId_partnerUserId_
			break;
		case "vse":
			token = token.split(":")[1];
			executionVariables.put("externalAccountId", token);
			executionVariables.put("servicePointId", token.split("-")[1]);
			metaTokenOFCreatedUser = ":" + executionVariables.get("externalAccountId"); //externalAccountId
			break;
		case "nve":
			token = token.split(":")[1];
			executionVariables.put("externalAccountId", SharedResources.generateRandomString("DDDDDDDDDDD"));
			executionVariables.put("servicePointId_"+gwsId, token); //token
			metaTokenOFCreatedUser = ":" + executionVariables.get("servicePointId_2"); //servicePointId
			break;
		case "pporres":
			token = token.split(":")[1];
			contractId = token.split("_")[0];
		...
		if(customerId!=null) executionVariables.put("customerId",customerId);
		if(contractId!=null) executionVariables.put("contractId",contractId);
		if(premiseId!=null)  executionVariables.put("premiseId", premiseId);
	...
	private String getRawData(GBConsumptionDataResponse gbRawConsumptionDataResponse, ExecutionVariables executionVariables, String utilityName, String contentFormat, DateFormat rawFileDateFormat, String gwsId, String meterFuel) {
		...
		case "ei":
			...
			for (Map.Entry<String, List<String>> entry : dateToValuesMap.entrySet()) {
				...
				updatedContext
						...
						.append(executionVariables.get("dataStreamId_" + gwsId)).append(PIPE)
		...
		case "pporres":
		case "desc":
			String dataStreamId = executionVariables.get("dataStreamId_" + gwsId).toString();
			for (GBConsumptionDataResponseEntry entry : gbRawConsumptionDataResponse) {
		...
		case "pnmtod":
			PIPE = ",";
			dataStreamId = executionVariables.get("dataStreamId_" + gwsId).toString();
		...
		case "sdgesmb":
			PIPE = ",";
			String servicePointIdNumber = executionVariables.get("servicePointIdNumber").toString();
		...
		case "pseg":
			...
			executionVariables.put("sdpId", sdpId);
		...
		case "honi":
			...
			break;
		case "vse":
			String externalAccountId = executionVariables.get("externalAccountId").toString();
			String servicePointId = executionVariables.get("servicePointId").toString();
			for (GBConsumptionDataResponseEntry entry : gbRawConsumptionDataResponse) {
				date = new Date(entry.getTime() * 1000L);
				/* for some pilots like NSP & Ameren, requires epochTimeStamp  */
				timeStamp = !rawFileDateFormat.format(date).isEmpty() ? rawFileDateFormat.format(date) : entry.getTime().toString();
			...
			break;
		case "nve":
			externalAccountId = executionVariables.get("externalAccountId").toString();
			servicePointId = executionVariables.get("servicePointId_"+gwsId).toString();
			PIPE=",";
			int lineNumber=1;
			for (GBConsumptionDataResponseEntry entry : gbRawConsumptionDataResponse) {
	...
	private String getInvoiceData(UtilityBillingDataResponse utilityBillingDataResponse,
		...
		case "honi":
			dataStreamId = executionVariables.get("dataStreamId_"+gwsId).toString();
			String partnerUserId = executionVariables.get("partnerUserId").toString();
			String contractAccountId = executionVariables.get("contractAccountId").toString();
	...
	private String getOpowerFileData(UtilityBillingDataResponse utilityBillingDataResponse, UserManagementUserRegistrationEntry userDetailsResponse, ExecutionVariables executionVariables, ExecutionContext1 executionContext, String utilityName, String meterType, String measurementType, DateFormat invoiceDateFormat, String invoiceValueSeparator, String contractStart, String contractEnd, String gwsId, String dateFormatStr) throws BidgelyExceptions {
		StringBuilder updatedContext = new StringBuilder();
		String PIPE=invoiceValueSeparator;
		...
		case "sdgesmb":
			String servicePointId = executionVariables.get("servicePointId").toString();
			String secondaryId = executionVariables.get("secondaryId").toString();
			PIPE="\u0009";
			...
			break;
		case "pnm":
			secondaryId = executionVariables.get("secondaryId").toString();
		...
		case "cva":
			PIPE="\t";
			secondaryId = executionVariables.get("secondaryId").toString();
			servicePointId = executionVariables.get("servicePointId").toString();
	...
	private String getUserFileData(UserManagementUserRegistrationEntry userDetailsResponse, ExecutionContext1 executionContext, String utilityName, ExecutionVariables executionVariables, String meterFuel, String contractStart, String contractEnd, String gwsId) throws IOException, BidgelyExceptions {
		String userFileData = "";
		String PIPE="|";
		...
		String dataStreamId = !gwsId.isEmpty() && executionVariables.containsKey("dataStreamId_"+gwsId)? executionVariables.get("dataStreamId_"+gwsId).toString(): "";
	...
	private String getMeterFileData(UserManagementUserRegistrationEntry userDetailsResponse, String utilityName, ExecutionVariables executionVariables, String meterFuel, String contractStart, String contractEnd, String gwsId){
		...
		case "edison":
		case "ei":
			meterFileData = customerId+"|"+contractId+"|"+premiseId+"|"+executionVariables.get("dataStreamId_"+gwsId)+"|"+measurementType+"|"+contractStart+"|"+contractEnd+"|"+ratePlanId+"|"+"|"+billCycleCode+"|"+"|"+hasSolar.toUpperCase()+"|"+meterType+"|"+"";
		}
