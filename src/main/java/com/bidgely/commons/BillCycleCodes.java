...
import com.bidgely.api.Apis;
import com.bidgely.commonExecutionResources.ExecutionContext1;
import com.bidgely.context.ContextKeys;
import com.bidgely.responses.BillCycleCodeWithIdentifierResponse;
...
public class BillCycleCodes extends BaseBidgelyApi<BillCycleCodeWithIdentifierResponse> {
	...
	//	private final String billCycleCodePrefix;
	...
	public BillCycleCodes(String beUrl, IAuthTokenManager authTokenManager)
		...
		super(beUrl, authTokenManager, Apis.GetUriFor.GET_BILLING_CYCLE_WITH_IDENTIFIER_FOR_PILOT, BillCycleCodeWithIdentifierResponse.class);
		this.baseUrl = beUrl;
		this.authTokenManager = authTokenManager;
	}

	/**
	 * This method gets the bill cycle code for the given date
	 *
	 * @param date
	 * @return
	 * @throws BidgelyExceptions
	 */
	public String getBillingCycleCodeAt(ExecutionContext1 executionContext,
			int date, int pilotId, String utilityName)
			throws BidgelyExceptions {

		String billCycleCodePrefix = executionContext.getProperty(ContextKeys.UserDefaults_BillCycleCodePrefix);
		String timeZone = executionContext.getProperty(ContextKeys.UserDefaults_Timezone);
		if (date > 31 || date < 1) {
		...
		if (!checkIfBillingCycleCodeExists(billingCycleCode, pilotId)) {
			boolean transitionBillingEnabled = Boolean.valueOf(executionContext.getProperty(ContextKeys.UserDefaults_TransitionBillingEnabled));
